...
export default class WindowFrame{
  constructor(){
    ...
    //Set up dragging
    this.ClickID = AddEventListener(this.TitleBarElement, "mousedown", function(Event){
      Event.preventDefault();
      this.Dragging = true;
    }.bind(this));
    this.MoveID = AddEventListener(window, "mousemove", function(Event){
      if(this.Dragging){
        Event.preventDefault();
    ...
    this.UnclickID = AddEventListener(window, "mouseup", function(Event){
      if(this.Dragging){
        Event.preventDefault();
        this.Dragging = false;
      } else if(this.Resizing){
    ...
    this.WindowResizeID = AddEventListener(window, "resize", function(Event){
      this.SetPosition(this.PositionX, this.PositionY);
    }.bind(this));
  ...
  Destroy(){
    RemoveEventListener(this.ClickID);
    RemoveEventListener(this.MoveID);
    RemoveEventListener(this.UnclickID);
    RemoveEventListener(this.WindowResizeID);
